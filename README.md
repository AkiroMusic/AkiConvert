...
src/
├── main/               # Electron 主进程
│   ├── ipc/            # IPC 处理器（转换、对话框、设置、历史记录等）
│   ├── ffmpeg.ts       # FFmpeg/FFprobe 封装
│   ├── ffmpeg-path.ts  # 平台感知的 FFmpeg 二进制路径解析
│   ├── ffmpeg-check.ts # FFmpeg 健康检查
│   ├── kggKeys.ts      # 酷狗解密密钥管理
│   ├── history.ts      # 转换历史持久化
│   ├── simpleStore.ts  # 基于 JSON 的键值存储
...
│       └── styles/     # CSS 令牌 + Tailwind 工具类
├── core/               # 共享核心逻辑
│   ├── decoders/       # 各格式解密实现
│   ├── ncmDecrypt.ts   # 网易云音乐（.ncm）解密
│   ├── id3Writer.ts    # ID3v2 标签写入器
│   ├── template.ts     # 文件名模板引擎
│   ├── types.ts        # 共享类型定义
│   └── supportedFormats.ts # 格式支持矩阵
