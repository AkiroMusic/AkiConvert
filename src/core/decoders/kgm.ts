...
export function decryptBuffer(kgmBuf: Uint8Array): DecoderResult {
  ...
  let isVpr: boolean
  if (buffersEqual(header, KGM_HEADER)) {
    isVpr = false
  } else if (buffersEqual(header, VPR_HEADER)) {
    isVpr = true
  } else {
  ...
  key.set(kgmBuf.slice(0x1C, 0x2C), 0)
  key[16] = 0

  const audio = new Uint8Array(kgmBuf.slice(headerLen))
